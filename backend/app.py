...
# this will create a file in the root directory of the applicaiton
...
class TrackedProducts(db.Model):
    ...
    name = db.Column(db.String(1000))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    tracked = db.Column(db.Boolean, default=True) # Tracking under process (True)
...
def submit_results():
    ...
    db.session.commit()
    response = {'messgae': 'data recieved successfully'}
    return jsonify(response), 200 

@app.route('/uniqeue_search_texts', methods=['GET'])
def get_uniquq_search_texts():
    get_uniquq_search_texts = db.session.query(ProductResult.search_text).distinct().all()
    uniqeue_search_texts = [text[0] for text in uniqeue_search_texts] 

    return jsonify(uniqeue_search_texts)
...
@app.route('/all-results', method=['GET'])
def get_results():
...
@app.route('/start-scrapper', method=['POST'])
def start_scrapper():
    ...
    response = {'message': 'scrapper started'}
    return jsonify(response), 200 

@app.route('/add-tracked-products', method=['POST'])
...
def toggle_tracked_products(product_id):
